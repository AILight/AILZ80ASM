...
## ラベル
ラベルは、ネームスペース、標準ラベル、ローカルラベルで指定します。ネームスペースを指定しない場合には、 'NAME_SPACE_DEFAULT' が割り当てられています。必要に応じて変更をしてください。
...
ラベルに使用できる文字は、英数字と_です。ただし、先頭が数字もしくは数値に変換できる文字列は使用できません。大文字と小文字は区別されません。
