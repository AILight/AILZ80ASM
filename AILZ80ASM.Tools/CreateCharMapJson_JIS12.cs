...
{
    public static class CreateCharMapJson_JIS12
    {
        private static readonly int[][] KANJI_Tables = new[]
        {
new [] { 0x8140, 0x817e},
            new [] { 0x8180, 0x81ac},
            new [] { 0x81b8, 0x81bf},
        ...
        public static void JIS()
        {
            ...
            foreach (var item in KANJI_Tables)
            {
                for (var index = item[0]; index <= item[1]; index++)
                {
                    ...
                    bytes[0] *= 2;
                    bytes[0] += 0x21;
                    if (bytes[1] >= 0x9F && bytes[1] <= 0xFF)
                    {
                        bytes[0]++;
                    }
                    else if (bytes[1] >= 0x01 && bytes[1] <= 0x1E)
                    {
                        bytes[0] = 0xFF;
                    }
                    // 2バイト目
